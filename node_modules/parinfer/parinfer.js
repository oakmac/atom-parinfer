...
function getInitialResult(text, options, mode, smart) {
  ...
  var result = {
    ...
    commentChar: SEMICOLON     // default comment character
  };
  ...
  if (options) {
    ...
    if (isString(options.commentChar))         { result.commentChar        = options.commentChar;  }
  }
...
function onChar(result) {
  ...
  var commentChar = result.commentChar
  ...
  else if (ch === commentChar)  { onComment(result); }
...
function checkIndent(result) {
  ...
  else if (result.ch === result.commentChar) {
