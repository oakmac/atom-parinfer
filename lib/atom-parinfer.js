...
function runParinfer(editor) {
  ...
  if (editorStates[editorId] === INDENT_MODE) {
    inferredText = parinfer.inferParens(textToInfer, adjustedCursorPosition);
  }
  else if (editorStates[editorId] === PAREN_MODE) {
    inferredText = parinfer.setIndentation(textToInfer, adjustedCursorPosition);
  }
...
function statusBarLink(state) {
  var txt;
  if (state === DISABLED) txt = 'Parinfer: Off'
...
function panelChanged() {
  ...
  // exit if there is no editor or not one we care about
  if (! editor || editorStates.hasOwnProperty(editor.id.toString()) !== true) return;
...
function newTextEditorOpened(editor) {
  ...
  var currentText = editor.getText();
      indentedText = parinfer.setIndentation(currentText);
