...
const defaultFileExtensionConfigFile = '# one file extension per line\n' + defaultFileExtensions.join('\n');
...
function loadFileExtensionsToWatch() {
  //fs.ensureFileSync(fileExtensionConfigFile);
  fs.readFile(fileExtensionConfigFile, utf8, function(err, txt) {
    if (err) {
      createDefaultFileExtensionConfig(loadFileExtensionsToWatch);
    }
    else {
...
function newTextEditorOpened(editor) {
  ...
  fileExtensionsToWatch.forEach(function(extension) {
