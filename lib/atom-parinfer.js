...
// a cache of the previous parinfer executions by editor id
// prevents double-inferring the same text/cursor and causing an infinite loop
var lastParinferRun = {};
...
function runParinfer(editor) {
  ...
  // TODO: do not run parinfer if there are multiple cursors?
  // TODO: need to run parinfer when the cursor changes position
  ...
  if (lastParinferRun.hasOwnProperty(editorId) === true &&
      lastParinferRun[editorId].inferredText === textToInfer &&
      lastParinferRun[editorId].adjustedCursorRow === adjustedCursorPosition.row &&
      lastParinferRun[editorId].adjustedCursorColumn === adjustedCursorPosition.column) {
    return;
  }

  var inferredText = parinfer.formatText(textToInfer, adjustedCursorPosition);
  if (inferredText) {
    editor.setTextInBufferRange([[startRow, 0], [endRow, 0]], inferredText);
    editor.setCursorBufferPosition(cursorPosition);

    // save this run in the cache
    lastParinferRun[editorId] = {
      inferredText: inferredText,
      adjustedCursorRow: adjustedCursorPosition.row,
      adjustedCursorColumn: adjustedCursorPosition.column
    };
  }
}
